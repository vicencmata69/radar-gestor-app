...
function GestorTab({refreshKey=0}){
  ...
  const onSubmit=()=>{if(!form.licitacio&&!form.client)return;let f=form;const propLike=f.estat==="PROPOSTA"||f.estat==="NO PROPOSTA";if(!propLike&&!f.codi_obra)f={...f,codi_obra:genCode(lic)};else if(propLike&&f.codi_obra)f={...f,codi_obra:""};if(f!==form)setForm(f);const dups=findDups(f,editId);if(dups.length>0){setDupModal({dups});return;}doSave(f,editId);};
    ...
    <div className="max-w-2xl mx-auto p-4">
      ...
      <div className="bg-stone-50 border border-gray-200 rounded-xl p-4 shadow-sm">
        <div className="grid grid-cols-2 gap-3">
          ...
          <FR label="Estat"><select className={inp} value={form.estat} onChange={e=>{const ne=e.target.value;setForm(p=>{const next={...p,estat:ne};if(ne!=="PROPOSTA"&&ne!=="NO PROPOSTA"&&!p.codi_obra)next.codi_obra=genCode(lic);return next;});}}>{ESTATS.map(s=><option key={s}>{s}</option>)}</select></FR>
