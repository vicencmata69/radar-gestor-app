...
export default function App(){
  ...
  const guardarAlGestor=useCallback(async r=>{
    try{
      ...
      // Format visita d'obra (sempre, encara que no sigui obligatòria) com PRIMER concepte dels comentaris.
      const formatVisita=v=>{
        if(!v||(typeof v==="string"&&(v.toLowerCase()==="no"||v.trim()===""))){
          return"🏗️ VISITA D'OBRA: No prevista";
        }
        if(typeof v==="string")return`🏗️ VISITA D'OBRA: ${v}`;
        const parts=["🏗️ VISITA D'OBRA:"];
        parts.push(v.obligatoria===true?"OBLIGATÒRIA ⚠️":(v.data||v.lloc)?"facultativa":"No prevista");
