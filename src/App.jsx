...
function GestorTab({refreshKey=0}){
  ...
  const onSubmit=()=>{if(!form.licitacio&&!form.client)return;let f=form;const propLike=f.estat==="PROPOSTA"||f.estat==="NO PROPOSTA";if(f.estat==="EN ESTUDI"&&!f.codi_obra)f={...f,codi_obra:genCode(lic)};else if(propLike&&f.codi_obra)f={...f,codi_obra:""};if(f!==form)setForm(f);const dups=findDups(f,editId);if(dups.length>0){setDupModal({dups});return;}doSave(f,editId);};
  ...
  const exportExcel=data=>{
    const cols=["CODI OBRA","LICITACIO","CLIENT","PRIVAT PUBLIC","POBLACIO","ESTAT","DATA PRESENTACIO","TERMINI","IMPORTE PEC Sin IVA","CLASSIF","CRITERIS PUNTUACIO","AVAL","APERTURA","COMENTARIOS","LINK ORGANISME"];
    const flds=["codi_obra","licitacio","client","public_privat","poblacio","estat","data_presentacio","termini","import_pec_sense_iva","classificacio","criteris_puntuacio","aval","apertura","comentaris","link_obra"];
    const IC=8;
    ...
    <div className="max-w-2xl mx-auto p-4">
      ...
      <div className="bg-stone-50 border border-gray-200 rounded-xl p-4 shadow-sm">
        <div className="grid grid-cols-2 gap-3">
          ...
          <FR label="Estat"><select className={inp} value={form.estat} onChange={e=>{const ne=e.target.value;setForm(p=>{const next={...p,estat:ne};if(ne==="EN ESTUDI"&&!p.codi_obra)next.codi_obra=genCode(lic);return next;});}}>{(editId===null?["PROPOSTA","EN ESTUDI"]:ESTATS).map(s=><option key={s}>{s}</option>)}</select></FR>
          ...
          <FR label="Link Organisme" span2><input className={inp} value={form.link_obra} placeholder="https://www.ajuntament... (web de l'organisme)" onChange={e=>setForm({...form,link_obra:e.target.value})} /></FR>
        </div>
  ...
  return(
    <div className="p-3 max-w-full" onClick={()=>setShowExport(false)}>
      ...
      {filtered.length===0?<div className="text-center py-16 text-gray-400"><div className="text-4xl mb-2">📋</div><div className="font-semibold">{lic.length===0?"Cap licitacio":"Cap resultat"}</div></div>
        ...
        })()}</tr></thead><tbody className="divide-y divide-gray-100">{filtered.map(l=>(<tr key={l.id} className="hover:bg-gray-50 align-top"><td className="px-2 py-2 whitespace-nowrap">{parseDT(l.data_presentacio)&&<button onClick={()=>addToCalendar(l,setCalSt)} title="Google Calendar" className="text-blue-400 hover:text-blue-600 mr-1">📅</button>}<button onClick={()=>onEdit(l)} className="text-blue-500 hover:text-blue-700 mr-1">✏️</button><button onClick={()=>onDel(l.id)} className="text-red-400 hover:text-red-600">🗑️</button></td><td className="px-2 py-2 font-mono text-gray-600 whitespace-nowrap">{l.codi_obra||"---"}</td><td className="px-2 py-2 font-medium text-gray-800"><div title={l.licitacio}>{l.licitacio||"---"}</div></td><td className="px-2 py-2 text-gray-700">{l.client||"---"}</td><td className="px-2 py-2"><select value={l.public_privat||tipus[0]} className={"text-xs font-semibold px-1.5 py-0.5 rounded border-0 cursor-pointer "+(l.public_privat===tipus[0]?"bg-blue-100 text-blue-700":"bg-orange-100 text-orange-700")} onChange={e=>{const updated={...l,public_privat:e.target.value};save(lic.map(x=>x.id===l.id?updated:x));saveOne(updated);}}>{tipus.map(t=><option key={t}>{t}</option>)}</select></td><td className="px-2 py-2 text-gray-600">{l.poblacio||"---"}</td><td className="px-2 py-2"><select value={l.estat} className={"text-xs font-semibold px-1.5 py-0.5 rounded-full border-0 cursor-pointer "+(EC[l.estat]||"bg-gray-100 text-gray-600")} onChange={e=>onEstat(l.id,e.target.value)}>{ESTATS.map(s=><option key={s}>{s}</option>)}</select></td><td className="px-2 py-2 text-gray-600"><div className="flex flex-col gap-1"><input type="datetime-local" className="text-xs border rounded px-1 py-0.5 w-full" value={(()=>{const v=l.data_presentacio||"";const m=v.match(/(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{4})(?:\s+(\d{1,2}):?(\d{2})?)?/);if(m)return`${m[3]}-${m[2].padStart(2,"0")}-${m[1].padStart(2,"0")}T${(m[4]||"00").padStart(2,"0")}:${(m[5]||"00").padStart(2,"0")}`;return"";})()} onChange={e=>{const d=e.target.value;if(!d)return;const dt=new Date(d);const formatted=`${String(dt.getDate()).padStart(2,"0")}/${String(dt.getMonth()+1).padStart(2,"0")}/${dt.getFullYear()} ${String(dt.getHours()).padStart(2,"0")}:${String(dt.getMinutes()).padStart(2,"0")}`;const updated={...l,data_presentacio:formatted};save(lic.map(x=>x.id===l.id?updated:x));saveOne(updated);}} /><InlineDateText key={l.id} initial={l.data_presentacio} onSave={v=>{const updated={...l,data_presentacio:v};save(lic.map(x=>x.id===l.id?updated:x));saveOne(updated);}}/></div></td><td className="px-2 py-2 text-gray-600">{l.termini||"---"}</td><td className="px-2 py-2 text-right font-medium text-gray-800 whitespace-nowrap">{(l.public_privat||tipus[0])!==tipus[0]?<input type="number" className="text-xs border rounded px-1 py-0.5 w-full text-right" defaultValue={l.import_pec_sense_iva||""} key={l.id+"-imp-"+l.import_pec_sense_iva} placeholder="Import" onBlur={e=>{const v=e.target.value?parseFloat(e.target.value):null;if(v!==(l.import_pec_sense_iva||null)){const updated={...l,import_pec_sense_iva:v};save(lic.map(x=>x.id===l.id?updated:x));saveOne(updated);}}}/>:(l.import_pec_sense_iva?fmt(l.import_pec_sense_iva):"---")}</td><td className="px-2 py-2 font-mono">{l.classificacio||"---"}</td><td className="px-2 py-2 text-gray-600">{l.criteris_puntuacio||"---"}</td><td className="px-2 py-2">{(()=>{const priv=(l.public_privat||tipus[0])!==tipus[0];if(priv)return <span className="text-xs text-gray-300">—</span>;const val=l.tecnica!=null?l.tecnica:(l.criteris_puntuacio&&/100\s*%?\s*(auto|sobre)/i.test(l.criteris_puntuacio)?false:true);return <select value={val?"Sí":"No"} className={"text-xs font-bold px-1.5 py-0.5 rounded border-0 cursor-pointer "+(val?"bg-red-500 text-white":"bg-green-100 text-green-700")} onChange={e=>{const updated={...l,tecnica:e.target.value==="Sí"};save(lic.map(x=>x.id===l.id?updated:x));saveOne(updated);}}><option>Sí</option><option>No</option></select>})()}</td><td className="px-2 py-2">{(()=>{
...
})()}</td><td className="px-2 py-2"><textarea rows={4} className="text-xs border rounded px-1 py-0.5 w-full resize-y" style={{minHeight:"60px",maxHeight:"200px"}} value={l.comentaris||""} placeholder="Comentaris" onBlur={e=>{if(e.target.value!==(l.comentaris||"")){const updated={...l,comentaris:e.target.value};save(lic.map(x=>x.id===l.id?updated:x));saveOne(updated);}}} onChange={e=>{save(lic.map(x=>x.id===l.id?{...l,comentaris:e.target.value}:x));}}/></td><td className="px-2 py-2 text-gray-600">{l.aval||"---"}</td><td className="px-2 py-2"><div className="flex items-center gap-1">{l.link_obra&&<a href={l.link_obra} target="_blank" rel="noreferrer" className="text-blue-500 hover:text-blue-700" title={l.link_obra}>🏛</a>}<input className="text-xs border rounded px-1 py-0.5 w-full" value={l.link_obra||""} placeholder="URL organisme" onBlur={e=>{if(e.target.value!==(l.link_obra||"")){const updated={...l,link_obra:e.target.value};save(lic.map(x=>x.id===l.id?updated:x));saveOne(updated);}}} onChange={e=>{save(lic.map(x=>x.id===l.id?{...l,link_obra:e.target.value}:x));}}/></div></td></tr>))}</tbody></table></div>}
